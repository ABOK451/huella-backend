...
export const obtenerRetos = async(req, res) => {
    try {
        ...
        res
            .status(500)
            .json({ error: "Error al obtener retos", detalle: error.message });
    }
};
...
export const obtenerRetosPorCategoria = async(req, res) => {
    try {
        ...
        res.status(500).json({
            error: "Error al obtener retos por categoría",
    ...
    try {
        ...
        if (retosAsignados < MAX_RETOS_DIARIOS) {
            ...
            if (nuevosRetosAleatorios.length > 0) {
                ...
                const instanciasCreadas = await RetoUsuario.bulkCreate(
                    nuevosRetosUsuario
                );
        ...
        res
            .status(500)
            .json({ error: "Error al obtener reto diario", detalle: error.message });
    }
};
...
export const completarReto = async(req, res) => {
    try {
        ...
        const retoUsuario = await RetoUsuario.findOne({
            where: { usuarioId, retoId: retoUsuarioId },
        });
        if (!retoUsuario)
        ...
        res
            .status(500)
            .json({ error: "Error al completar reto", detalle: error.message });
    }
};
...
export const obtenerEstadisticas = async(req, res) => {
    try {
        ...
        impactoData.forEach((r) => {
            console.log(
                `Reto: ${r.reto?.titulo}, CO2: ${r.reto?.impacto_co2}, Agua: ${r.reto?.impacto_agua}`
            );
            if (r.reto) {
        ...
        res
            .status(500)
            .json({ error: "Error al obtener estadísticas", detalle: error.message });
    }
};